...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-border shadow-sm">
      <div className="container mx-auto px-4 h-16 md:h-20 flex items-center justify-between">
        <div className="flex-1 flex items-center">
          <Link href="/" className="flex items-center gap-2 group w-fit">
            <div className="text-primary group-hover:scale-105 transition-transform">
              <svg viewBox="0 0 1010 416" fill="currentColor" className="h-6 md:h-8">
            ...
            <span className="font-heading font-bold text-sm md:text-xl uppercase tracking-wider hidden sm:block">BoostFix Usinagem</span>
          </Link>
        </div>

        <nav className="hidden md:flex flex-none items-center justify-center gap-6 font-semibold text-sm">
        ...
        <div className="flex-1 flex items-center justify-end gap-3 md:gap-4">
          ...
          <button
            ...
            className="relative flex items-center gap-2 font-semibold text-xs md:text-sm bg-text-main text-white px-3 md:px-4 py-2 hover:bg-primary transition-colors group"
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-4 h-4 md:w-5 md:h-5 group-hover:scale-110 transition-transform">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
            </svg>
            <span className="hidden sm:inline">Carrinho</span>
            <span className="bg-white text-text-main text-xs font-bold w-5 h-5 flex items-center justify-center ml-1">
              {cartCount}
            </span>
          </button>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-16 left-0 w-full bg-white border-b border-border shadow-lg z-50 p-4">
          <nav className="flex flex-col gap-3 font-semibold text-sm">
            <Link href="/" className="hover:text-primary transition-colors p-2 border-b border-border/50" onClick={() => setIsMobileMenuOpen(false)}>Início</Link>
            <Link href="/produtos" className="hover:text-primary transition-colors p-2 border-b border-border/50" onClick={() => setIsMobileMenuOpen(false)}>Loja</Link>
            <Link href="/produtos" className="hover:text-primary transition-colors p-2 border-b border-border/50" onClick={() => setIsMobileMenuOpen(false)}>Linha M8</Link>
            <Link href="/produtos" className="hover:text-primary transition-colors p-2 border-b border-border/50" onClick={() => setIsMobileMenuOpen(false)}>Linha M10</Link>
